...
def do_move(grid, x1, y1, x2, y2):
    ...
    """
    moving_sand = grid[y1][x1]
    grid[y2][x2] = moving_sand
    grid[y1][x1] = None
    return grid
    pass
...
def check_move(grid, x1, y1, x2, y2):
    ...
    right_x = x1 + 1
    down_y = y1 + 1
    if x2 == left_x and y2 == down_y:
        if grid[y1][left_x] != None:
            # print("Left is blocked")
            return False
    if x2 == right_x and y2 == down_y:
        if grid[y1][right_x] != None:
...
def do_gravity(grid, x, y):
    ...
    down_y = y + 1
    if grid[y][x] == 's':
        if check_move(grid, x, y, x, down_y):
            do_move(grid, x, y, x, down_y)
            return [x, down_y]
        if check_move(grid, x, y, left_x, down_y):
            do_move(grid, x, y, left_x, down_y)
            return [left_x, down_y]
        if check_move(grid, x, y, right_x, down_y):
            do_move(grid, x, y, right_x, down_y)
...
def do_brownian(grid, x, y, brownian):
    ...
    if grid[y][x] == 's':
        ...
        if num < brownian:
            ...
            if coin == 0:
                if check_move(grid, x, y, left_x, y):
                    do_move(grid, x, y, left_x, y)
            if coin == 1:
                if check_move(grid, x, y, right_x, y):
                    do_move(grid, x, y, right_x, y)
    return grid
    pass
...
def do_whole_grid(grid, brownian):
    ...
    for y in reversed(range(len_y)):
        for x in range(len_x):
            vari = do_gravity(grid, x, y)
            if vari is not None:
                # [new_x, new_y] = do_gravity(grid, x, y)
                do_brownian(grid, vari[0], vari[1], brownian)
    return grid
    pass
